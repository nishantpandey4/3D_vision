...
def evaluate_model(args):
    ...
    for step in range(start_iter, max_iter):
        ...
        #Extracting all the layers into a list. This is used for later visualization.
        ...
        img_step = step % 50 
        num = step // 50 
        #For each layer let's plot the output for the image.
        if img_step == 0:
            ...
            #plotting the each layers output.
            plot_pred(predictions[-1][0].cpu().detach().numpy(), output_path=f'Images/part6/q2-6-pred-{num}.png')
            plot(images_gt.squeeze().cpu().detach().numpy(), output_path=f'Images/part6/q_2-6-gt-{num}.png')
